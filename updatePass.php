		...
		$link = mysqli_connect("shareddb-n.hosting.stackcp.net","eattendance-313037e3ac","srijan1998","eattendance-313037e3ac");
				...
				if(mysqli_connect_error() != ""){
					...
					if(array_key_exists("update",$_POST)){
						...
						$query="SELECT `password` FROM `teacher` WHERE `id` = '".mysqli_real_escape_string($link,$id)."' LIMIT 1";
						$result=mysqli_query($link,$query);
						$row=mysqli_fetch_array($result);

						if( $row['0'] == $_POST['currPass']){
							...
							if( $_POST['newPass1'] != $_POST['newPass2'] ){
								...
								$pass= $_POST['newPass1'];
								$query= " UPDATE `teacher` SET `password` = '".mysqli_real_escape_string($link,$pass)."' WHERE `id` = '".$id."' ";
...
<? 
	...
	if( $error != ""){
		...
		echo "<div id='error' class= 'alert alert-dark' role='alert'> <strong>".$error." </strong></div>";
	}

	if( $success != ""){

		echo "<div id='error' class= 'alert alert-dark' role='alert'> <strong>".$success." </strong></div>";
	}
...
<input type="password" class="form-control" id="currentPass" name="currPass" placeholder="Enter current password">
</div>
<div class="form-group" style="text-align:left;>
...
<div class="form-group" style="text-align:left;>
	<label for="newPass"><strong>Confirm New Password</strong></label>
	<input type="password" class="form-control" id="newPass2" name="newPass2" placeholder="Enter new password">
</div>
